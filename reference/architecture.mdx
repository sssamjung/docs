...
<Frame caption="Pulse STO 플랫폼 전체 아키텍처">
  <img src="/images/diagrams/sto-whitepaper-large.png" alt="Pulse STO 플랫폼 아키텍처 다이어그램" />
</Frame>
...
```mermaid
flowchart TB
    Traffic["Traffic"] --> ALB["ALB<br/>(Application Load Balancer)"]
    ALB --> Inst1["Instance<br/>AZ-a"]
    ALB --> Inst2["Instance<br/>AZ-b"]
    ALB --> Inst3["Instance<br/>AZ-c"]
```
...
```mermaid
flowchart LR
    Master["Master<br/>(Writer)"] --> Slave1["Slave 1<br/>(Reader)"]
    Master --> Slave2["Slave 2<br/>(Reader)"]
```
...
```mermaid
flowchart LR
    Client["Client"] --> Redis["Redis<br/>(Cache)"]
    Redis -->|캐시 Miss| DB["Database"]
    Redis -->|캐시 Hit| Fast["Fast Return"]
    DB --> Redis
```
